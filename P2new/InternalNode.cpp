...
BTreeNode* InternalNode::rmChild(int pos)
{
    count --;
    BTreeNode *ptr;
    ptr = children[pos];
...
InternalNode* InternalNode::rmSibling()
{
//    rmRSibling();
    rmLSibling();
...
//    else
//    {
//        insert(((InternalNode*)leftSibling)->rmChild(leftSibling->getCount() - 1));
//        
//        if(parent)
//            parent->resetMinimum(this);
//        
...
InternalNode* InternalNode::rmLSibling()
{
  if(leftSibling->getCount() <= (internalSize + 1) / 2)
  {
      int i = 0;
...
InternalNode* InternalNode::rmRSibling(int pos)
{
  if(rightSibling->getCount() <= (internalSize + 1) / 2)
  {
      ...
      while (count - i - 1 >=0)
      {
          ((InternalNode*)rightSibling)->insert(children[count - i - 1]);
          i++;
      }
  ...
  else
  {
      ...
      insert(((InternalNode*)rightSibling)->rmChild(0));
      if(pos == 0)
          parent->resetMinimum(this);
      return NULL;
  }
